...
export class AuthService {
  async register(data: { email: string; password: string; username?: string; displayName?: string }): Promise<any> {
    ...
    } catch (err: any) { if (err.statusCode) throw err }
    const passwordHash = await bcrypt.hash(password, 12)
    try {
      await query('BEGIN')
      const result = await query(`INSERT INTO users (email, username, display_name, password_hash, plan, credits, is_active, created_at, updated_at) VALUES ($1, $2, $3, $4, 'FREE', 50, true, NOW(), NOW()) RETURNING id, email, username, display_name, plan, credits, created_at`, [email.toLowerCase(), username || null, displayName || null, passwordHash])
      const user = result.rows[0]
      await query(`INSERT INTO wallets (user_id, balance, total_funded, total_spent, created_at, updated_at) VALUES ($1, 0, 0, 0, NOW(), NOW())`, [user.id])
      try {
  ...
  async login(email: string, password: string, ipAddress?: string, userAgent?: string): Promise<any> {
    try {
      ...
      try {
        const failedAttempts = await query(`SELECT COUNT(*) as count FROM auth_sessions WHERE user_id = $1 AND expires_at > NOW() - INTERVAL '15 minutes' AND token_hash LIKE 'failed_%'`, [user.id])
        if (parseInt(failedAttempts.rows[0]?.count || '0') >= 5) throw createError('Too many failed attempts. Try again in 15 minutes.', 429)
      } catch (err: any) { if (err.statusCode) throw err }
      const isValid = await bcrypt.compare(password, user.password_hash)
      if (!isValid) {
        try { await query(`INSERT INTO auth_sessions (user_id, token_hash, ip_address, user_agent, expires_at, created_at) VALUES ($1, $2, $3, $4, NOW() + INTERVAL '15 minutes', NOW())`, [user.id, `failed_${Date.now()}`, ipAddress || null, userAgent || null]) } catch {}
        throw createError('Invalid credentials', 401)
      }
      await query('UPDATE users SET last_login_at = NOW() WHERE id = $1', [user.id])
      const token = this.generateToken(user)
      try { await query(`INSERT INTO auth_sessions (user_id, token_hash, ip_address, user_agent, expires_at, created_at) VALUES ($1, $2, $3, $4, NOW() + INTERVAL '7 days', NOW())`, [user.id, await bcrypt.hash(token, 10), ipAddress || null, userAgent || null]) } catch {}
  ...
  async logout(userId: string, _token: string): Promise<void> {
    try { await query('DELETE FROM auth_sessions WHERE user_id = $1 AND expires_at > NOW()', [userId]) } catch {}
  }
  ...
  private generateToken(user: any): string {
    return jwt.sign({ id: user.id, email: user.email, role: user.role || 'USER', plan: user.plan }, config.jwtSecret, { expiresIn: config.jwtExpiresIn as any })
  }

  async verifyToken(token: string): Promise<any> {
    try { return jwt.verify(token, config.jwtSecret) as any } catch { throw createError('Invalid or expired token', 401) }
  }
  ...
  async requestPasswordReset(email: string): Promise<void> {
    try {
      ...
      await query(`INSERT INTO password_resets (user_id, token_hash, expires_at, created_at) VALUES ($1, $2, NOW() + INTERVAL '1 hour', NOW())`, [userId, await bcrypt.hash(resetToken, 10)])
      try {
        const { notificationService } = await import('./notificationService')
        await notificationService.send(userId, { type: 'SECURITY_EVENT', data: { event: 'Password reset requested', resetToken: resetToken.substring(0, 10) + '...' } })
      } catch {}
    } catch (err) { console.error('[AUTH] Password reset error:', err) }
  }
  ...
  async resetPassword(token: string, newPassword: string): Promise<void> {
    ...
    try {
      ...
      await query('UPDATE users SET password_hash = $1, updated_at = NOW() WHERE id = $2', [await bcrypt.hash(newPassword, 12), decoded.userId])
