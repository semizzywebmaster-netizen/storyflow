...
async function verifyProject(userId: string, projectId: string) {
  const result = await query('SELECT id FROM projects WHERE id=$1 AND user_id=$2 AND deleted_at IS NULL', [projectId, userId])
...
router.get('/timeline/:projectId', async (req: AuthRequest, res, next) => {
  try {
    ...
    const scenes = await query(
      `SELECT id,title,scene_index AS "sceneIndex",duration_seconds AS "durationSeconds",transition
       FROM scenes WHERE project_id=$1 ORDER BY scene_index ASC LIMIT ${MAX_SCENES}`,
      [projectId]
    )
    const tracks = [{ id: 'main-video', type: 'VIDEO', name: 'Main Video', clips: scenes.rows.map((scene: any, index: number) => ({ id: scene.id, sceneId: scene.id, title: scene.title || `Scene ${index + 1}`, start: index * (scene.durationSeconds || SCENE_DURATION), duration: scene.durationSeconds || SCENE_DURATION, transition: scene.transition || 'CUT' })) }]
    const timeline = { version: 1, fps: 30, aspectRatio: '16:9', tracks }
    await query('INSERT INTO video_timelines(project_id,user_id,timeline) VALUES($1,$2,$3) ON CONFLICT(project_id) DO NOTHING', [projectId, userId, JSON.stringify(timeline)])
    return res.json({ success: true, data: { timeline, updatedAt: new Date().toISOString() } })
  } catch (error) { next(error) }
})

router.put('/timeline/:projectId', async (req: AuthRequest, res, next) => {
  try {
    const userId = req.user!.id
    const projectId = req.params.projectId
    await verifyProject(userId, projectId)
    ...
    if (incoming.tracks.length > 10) return next(createError('A maximum of 10 timeline tracks is supported', 400))
    const timeline = { version: 1, fps: Number(incoming.fps) || 30, aspectRatio: incoming.aspectRatio || '16:9', tracks: incoming.tracks }
    const result = await query(
      `INSERT INTO video_timelines(project_id,user_id,timeline) VALUES($1,$2,$3)
       ON CONFLICT(project_id) DO UPDATE SET timeline=EXCLUDED.timeline,updated_at=NOW()
       RETURNING timeline,updated_at AS "updatedAt"`,
      [projectId, userId, JSON.stringify(timeline)]
    )
...
router.post('/render', async (req: AuthRequest, res, next) => {
  ...
  try {
    ...
    const scenes = await query(
      `SELECT s.id, s.scene_index, a.url FROM scenes s
       INNER JOIN projects p ON p.id=s.project_id AND p.user_id=$2 AND p.deleted_at IS NULL
       INNER JOIN LATERAL (SELECT url FROM assets WHERE scene_id=s.id AND user_id=$2 AND type='IMAGE' AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 1) a ON TRUE
       WHERE s.project_id=$1 ${sceneFilter} ORDER BY s.scene_index ASC LIMIT ${MAX_SCENES}`,
      params
    )
    ...
    await execFileAsync('ffmpeg', ['-hide_banner','-loglevel','error','-y','-f','concat','-safe','0','-i',concatFile,'-vf','scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2,format=yuv420p','-r','30','-movflags','+faststart','-c:v','libx264',outputFile], { timeout: 10 * 60 * 1000, maxBuffer: 1024 * 1024 * 4 })
    ...
    await query(`INSERT INTO assets (user_id,project_id,type,url,file_size,mime_type,duration_seconds,model,metadata) VALUES ($1,$2,'VIDEO',$3,$4,'video/mp4',$5,'ffmpeg-storyflow',$6)`, [userId, projectId, stored.url, stored.size, scenes.rows.length * SCENE_DURATION, JSON.stringify({ generationId, sceneCount: scenes.rows.length, storageKey: key })])
    await query(`UPDATE generations SET status='COMPLETED',credits_consumed=$2,result_url=$3,metadata=metadata || $4::jsonb,updated_at=NOW() WHERE id=$1`, [generationId, VIDEO_CREDITS, stored.url, JSON.stringify({ storageKey: key })])
    ...
    try { await creditService.releaseReservedCredits(userId, VIDEO_CREDITS, generationId, 'Video render failed'); await query(`UPDATE generations SET status='FAILED',error_message=$2,updated_at=NOW() WHERE id=$1`, [generationId, String(error?.message || 'Video render failed').slice(0, 1000)]) } catch (finalizeError) { console.error('[VIDEO] Failed to finalize failed generation', finalizeError) }
    if (String(error?.message || '').includes('ffmpeg')) return next(createError('Video rendering is temporarily unavailable because FFmpeg is not installed on the server.', 503))
    return next(error)
...
router.get('/generation/:id', async (req: AuthRequest, res, next) => {
  try {
    const result = await query(`SELECT id,project_id AS "projectId",type,status,credits_reserved AS "creditsReserved",credits_consumed AS "creditsConsumed",result_url AS "resultUrl",error_message AS "errorMessage",metadata,created_at AS "createdAt",updated_at AS "updatedAt" FROM generations WHERE id=$1 AND user_id=$2 AND type='VIDEO'`, [req.params.id, req.user!.id])
